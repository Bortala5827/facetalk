...
ok('60 秒试音卡存在', /60\s*秒试音|试音互评/.test(pair));
ok('闻声识搭子预告已移除（按需求删除下一版提醒）', !/闻声识搭子/.test(pair));
ok('声纹节奏特征文案已移除', !/声纹节奏特征/.test(pair));
ok('顶栏齿轮已移除，但面试间内 AI 设置入口（iv-open-settings）保留', /id=["']iv-open-settings["']/.test(pair) && !/id=["']open-settings["']/.test(pair));
ok('iv-card 加 .iv-testing 切换', /ivc\.classList\.toggle\(['"]iv-testing['"]/.test(pair));
ok('注释 v2.2 「功能测试中」灰化', /v2\.2.*功能测试中|功能测试中.*v2\.2/.test(pair));
ok('注释 v2.2 「闻声识搭子」预告已移除', !/v2\.2.*闻声识搭子|闻声识搭子.*v2\.2/.test(pair));
